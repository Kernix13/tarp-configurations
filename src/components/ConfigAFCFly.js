...
function Config_AF_CFly() {
  ...
  class Config_AF_CFly {
    ...
    calcs() {
      ...
      outputObj = tarpSize.concat({ sleepClear, cover, coverClear, ridgeHeight, sitTarpHtClear, chairTarpHtClear, angle: this.alpha, angle2: this.beta, configName: this.configName, ridgeHt, sleepDiagClr, configImg });
          ...
          <div key={index} className="flex flex-col justify-center items-center my-8 bg-slate-100 border border-solid border-slate-400 sm:flex-row">
            ...
            <div className="p-4">
              ...
              <p className="mb-1 text-base lg:text-xl">
                ...
                to <span className="font-bold">{type[2].ridgeHeight}</span> inches. The 1st <span className="italic">lean angle</span> is {type[2].angle}&deg;, the 2nd is {type[2].angle2}&deg;.
              </p>
              <p className="mb-1 text-base lg:text-xl">
                Sit under tarp?{" "}
